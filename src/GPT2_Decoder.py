...
from transformers import GPT2Tokenizer, GPT2ForSequenceClassification, TrainingArguments, Trainer, TrainerCallback
...
k = 50000 # Number of training samples per label
base_model = "gpt2"
...
tokenizer.pad_token = tokenizer.eos_token  # GPT-2 does not have a pad token, so use eos_token
...
model = GPT2ForSequenceClassification.from_pretrained(base_model, num_labels=output_dim).to(device)
