...
os.environ["HF_TOKEN"] = "Your API Token"
...
from transformers import AutoModelForSequenceClassification,AutoModelForSeq2SeqLM, AutoTokenizer, TrainingArguments, Trainer, TrainerCallback
from transformers import EvalPrediction, T5ForConditionalGeneration, T5Tokenizer
...
type_of_label = [4,21,30,22]  # or any other labels you want to filter
k = 50000
base_model = "t5-small"
...
# Define data load
def load_data(file_path, type_of_data, type_of_label, k):
    ...
    for label in type_of_label:
        ...
        train_data = [item for item in label_data if item['train'] == 1]
        test_data = [item for item in label_data if item['train'] == 0]
        
        # Limit the number of training samples per label to k
        if len(train_data) > k:
            train_data = train_data[:k]
        
        train_texts += [item[type_of_data] for item in train_data]
        train_labels += [item['label'] for item in train_data]
        test_texts += [item[type_of_data] for item in test_data]
...
train_texts, train_labels, test_texts, test_labels = load_data(data_file, type_of_data, type_of_label, k)
...
train_encodings = tokenizer(["classify: " + text for text in train_texts], truncation=True, padding=True, max_length=400, return_tensors="pt")
test_encodings = tokenizer(["classify: " + text for text in test_texts], truncation=True, padding=True, max_length=400, return_tensors="pt")
...
model = AutoModelForSequenceClassification.from_pretrained(base_model, num_labels=len(label_mapping)).to(device)

output_dir = f'{type_of_data}_{"_".join(map(str, type_of_label))}_k{str(k)}'
os.makedirs(output_dir, exist_ok=True)

def compute_metrics(pred):
    # Extract the logits from the tuple if needed
...
eval_metrics = compute_metrics(EvalPrediction(predictions=eval_results.predictions, label_ids=eval_results.label_ids))
...
# Generate confusion matrix and classification report using predictions
predictions = eval_results.predictions[0] if isinstance(eval_results.predictions, tuple) else eval_results.predictions
predicted_labels = predictions.argmax(-1)
decoded_true_labels = [reverse_label_mapping[label] for label in eval_results.label_ids]
decoded_predictions = [reverse_label_mapping[pred] for pred in predicted_labels]
